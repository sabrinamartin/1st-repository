...
public class CardSoln3 {
	...
	Cardx card = new Cardx(1, 1);
	card.print();
	String s = card.toString();
	System.out.println(s);
	System.out.println(card);

	Cardx card2 = new Cardx(1, 1);
	System.out.println(card.equals(card2));

	Deckx deck = new Deckx();
     ...
     */
    public static void checkSorted(Deckx deck) {
	for (int i=0; i<51; i++) {
...
/*
 * A Cardx represents a playing card with rank and suit.
 */
class Cardx {
    int suit, rank;
     ...
     */
    public Cardx(int suit, int rank) { 
     ...
     */
    public boolean equals(Cardx that) {
     ...
     */
    public int compareTo(Cardx that) {
...
class Deckx {
     ...
     */
    public Deckx() {
	this.cards = new Cardx [52];
	
	int index = 0;
	for (int suit = 0; suit <= 3; suit++) {
	    for (int rank = 1; rank <= 13; rank++) {
		this.cards[index] = new Cardx(suit, rank);
		index++;
 	    }
	}
    }
     ...
     */
    public int findCard (Cardx card) {
	for (int i = 0; i< cards.length; i++) {
     ...
     */
    public int findBisect(Cardx card, int low, int high) {
     ...
     */
    public Deckx subdeck(int low, int high) {
	Deckx sub = new Deckx(high-low+1);
     ...
     */
    public Deckx merge(Deckx d2) {
	// create the new deck
	Deckx result = new Deckx (this.cards.length + d2.cards.length);
	...
	Deckx d1 = subdeck(0, mid);
	Deckx d2 = subdeck(mid+1, cards.length-1);
